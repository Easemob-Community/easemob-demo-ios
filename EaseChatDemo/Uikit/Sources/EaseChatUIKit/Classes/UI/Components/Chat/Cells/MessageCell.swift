...
@objcMembers open class MessageCell: UITableViewCell {
    ...
    open func updateAxis(entity: MessageEntity) {
        ...
        if entity.message.direction == .receive {
            ...
            if Appearance.chat.contentStyle.contains(.withMessageReaction) {
                self.reactionView.frame = CGRect(x: Appearance.chat.bubbleStyle == .withArrow ? self.bubbleWithArrow.frame.minX:self.bubbleMultiCorners.frame.minX, y: entity.height-entity.bottomAccessoryHeight-(Appearance.chat.contentStyle.contains(.withDateAndTime) ? 24:2)-(Appearance.chat.contentStyle.contains(.withMessageReaction) ? reactionContentHeight:2), width: reactionWidth+30, height: reactionContentHeight)
            }
            self.messageDate.frame = CGRect(x: Appearance.chat.contentStyle.contains(where: { $0 == .withAvatar }) ? self.avatar.frame.maxX+12:(self.editMode ? self.checkbox.frame.maxX+12:12), y: entity.height-24-entity.bottomAccessoryHeight, width: 120, height: 16)
        } else {
            self.status.isHidden = false
            if self.editMode {
                self.checkbox.frame = CGRect(x: 12, y: entity.height - 14 - entity.bottomAccessoryHeight - (Appearance.chat.contentStyle.contains(where: { $0 == .withDateAndTime }) ? 16:2) - 28 - (Appearance.chat.contentStyle.contains(where: { $0 == .withMessageThread }) ? (self.topicView.isHidden ? 0:topicHeight):0) - (Appearance.chat.contentStyle.contains(where: { $0 == .withMessageReaction }) ? reactionContentHeight:0), width: 28, height: 28)
            }
            self.avatar.frame = CGRect(x: ScreenWidth-40, y: 0, width: 36.5, height: 36.5)
            self.nickName.frame = CGRect(x: Appearance.chat.contentStyle.contains(where: { $0 == .withAvatar }) ? self.avatar.frame.minX-limitBubbleWidth-12:ScreenWidth-limitBubbleWidth-12, y: 0, width: limitBubbleWidth, height: 16)
            self.messageDate.frame = CGRect(x: Appearance.chat.contentStyle.contains(where: { $0 == .withAvatar }) ? (self.avatar.frame.minX-12-120):(ScreenWidth-132), y: entity.height-16-entity.bottomAccessoryHeight, width: 120, height: 16)
            self.messageDate.textAlignment = .right
            self.nickName.textAlignment = .right
            if Appearance.chat.contentStyle.contains(.withReply) {
                self.replyContent.frame = CGRect(x: Appearance.chat.contentStyle.contains(where: { $0 == .withAvatar }) ? self.avatar.frame.minX-entity.replySize.width-12:ScreenWidth-12-entity.replySize.width, y: entity.showNickName() ? self.nickName.frame.maxY+2:0, width: entity.replySize.width, height: entity.replySize.height)
            }
            ...
            self.messageDate.frame = CGRect(x: Appearance.chat.contentStyle.contains(where: { $0 == .withAvatar }) ? (self.avatar.frame.minX-12-120):(ScreenWidth-132), y: entity.height-24-entity.bottomAccessoryHeight, width: 120, height: 16)
